var page_actions = function(){
    ...
    $(".lockscreen-box .user_signin").on("click",function(){
        $(".sign-in").show();
        $(this).remove();
        $(".user").hide().find("img").attr("src","../img/clnt_img/no-image.jpg");
        $(".user").show();
        return false;
    });
    /* END LOCK SCREEN */
...
function page_content_onresize(){
    ...
    if(vpW < 1200){
		...
		if(vpW <= 990){$('.lgpgslimg').hide();}else{$('.lgpgslimg').fadeIn('slow');}
		//End AB Editing
...
/*window.onload=function(){
	var pgwh=Math.max(document.documentElement.clientWidth, window.innerWidth || 0)
  if(pgwh<1200){
		$(".page-container").removeClass("page-navigation-toggled");
	}
};*/
